...
from data_collator import EmolexDataCollatorForLanguageModeling

from modeling.modeling_bert import EmoBertForMaskedLM
...
emolex2id = {}
emolex2id['None'] = 0
for id, feature in enumerate(emo_lexicon):
    emolex2id[feature] = id+1
...
def main():
    ...
    model = EmoBertForMaskedLM.from_pretrained(
        model_checkpoint,
        config=config,
        output_dim=len(emolex2id)
    )

    # model.resize_token_embeddings(len(tokenizer))
    model.resize_embedding_and_fc(len(emolex2id))
    ...
    data_collator = EmolexDataCollatorForLanguageModeling(
        tokenizer=tokenizer, 
